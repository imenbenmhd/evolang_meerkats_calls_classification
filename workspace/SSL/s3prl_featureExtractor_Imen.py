...
def extract_features(model, wav_paths, wav_ids, batch_size, layer_id, device="cuda"):
    ...
    for bid in range(0, num_wavs, batch_size):
        ...
        padded_wavs, wavs_len, ids = get_batch(wav_file_paths, wav_ids, bid, end_id)
...
if __name__ == "__main__":
    ...
    if args.npickle == -1:
        ...
        for pid in tqdm(range(0, num_wav_files, npickle)):
            ...
            features_dict = extract_features(
                model, wav_files, wav_ids, args.bs, layer_id_for_extraction, device
            )
            feature_file = "{}/{}_{}.pkl".format(args.expdir, args.picklename, file_id)
            dump_features(features_dict, feature_file)
